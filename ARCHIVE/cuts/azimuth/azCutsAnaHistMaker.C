...
void azCutsAnaHistMaker(const char* infile = "infile-list.txt", const char* outfile="Analysis-RunXXX.root", int nCutEvt = -1, int azMin = -180,int azMax = 180) {
...
double yStart = 0,yStop = 0;
double zStart = 0,zStop = 0;
...
while (true){
    ...
    if ((int)hAzimuth->GetEntries()%1000==0)
    {
		cout <<"#### Event " << aMgr.EventNumber() << " ###" << endl;
		cout << "hAzimuth->GetEntries()" << '\t' << hAzimuth->GetEntries()<<endl; 
    }
	...
	for(iTrk = 0; iTrk < aMgr.NTracks(); iTrk++){
		...
		xStop = currtrk->TrackEndPoint(0,Fit);
		yStop = currtrk->TrackEndPoint(1,Fit);
		zStop = currtrk->TrackEndPoint(2,Fit);
		...
		if (azMin > azMax)
			azCut = ((azMin < phi || phi < azMax));
		else
			azCut = (azMin < phi && phi < azMax);			

		if(azCut && zStop > 0 && zStart > 0)
		{
			// fill histograms
	...
	if(!(nCutEvt > hAzimuth->GetEntries() || nCutEvt==-1 ))
	{
		cout << "hAzimuth->GetEntries()" << '\t' << hAzimuth->GetEntries()<<endl; 
		break;
	}	
