...
void zCutHistMaker(const char* infile = "infile-list.txt", const char* outfile="Analysis-RunXXX.root", int nevt = -1) {
...
AnaH2D* hStopXY = new AnaH2D("hStopXY","X-Y stop position of track",1000,-10,10,1000,-10,10);
AnaH1D* hStartZ = new AnaH1D("hStartZ","Z start position of track",1000,-10,10);
AnaH1D* hStopZ = new AnaH1D("hStopZ","Z stop position of track",1000,-10,10);
...
while (true){
	...
	for(iTrk = 0; iTrk < aMgr.NTracks(); iTrk++){
		...
		cut = ( 1 < sqrt(pow(xStart-0.17,2) + pow(yStart+0.1,2)) );
