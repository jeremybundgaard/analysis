...
void azimuthalDependence_dNdPhi(const int preamp, const int phiLoopMin, const int phiLoopMax, const int phiLoopStep) {
...
if (preamp==1)
{
	sprintf(cftFileName,"~/niffte/trees/cfCFT.1stPreamp.trees.1865.73.83.root");
	sprintf(noMaskFileName,"~/niffte/trees/cfNoMask.1stPreamp.trees.1850-56.66-72.74-82.root");
...
{
	sprintf(cftFileName,"~/niffte/trees/cfCFT.2ndPreamp.trees.2068-9.root");
	sprintf(noMaskFileName,"~/niffte/trees/cfNoMask.2ndPreamp.trees.2057-61.root");
...
for (int phi = phiLoopMin; phi < phiLoopMax; phi+=phiLoopStep){
	...
	phiCut.str("");
    phiCut<< radPhiMin-TMath::Pi() <<" < direction.Phi() && direction.Phi() < "<< radPhiMax-TMath::Pi() <<" && bestFit";
