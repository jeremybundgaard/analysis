...
void stepThruFragCutTree(const char* cftFile,const char* noMaskFile, int thetaMin=-1, int thetaMax=-1){

TFile * cftFileHandle = new TFile(cftFile);
TFile * noMaskFileHandle = new TFile(noMaskFile);
TTree * cftDataTree = (TTree*)cftFileHandle->Get("datatree");
TTree * noMaskDataTree = (TTree*)noMaskFileHandle->Get("datatree");
TH2D * cftLengthvsADC = new TH2D("cftLengthvsADC","Track Length vs. Energy",40000,0,40000,1000,0.0,10.0);
TH2D * noMaskLengthvsADC = new TH2D("noMaskLengthvsADC","Track Length vs. Energy",40000,0,40000,1000,0.0,10.0);

stringstream thetaCut;
double cosThetaMin=TMath::Cos(thetaMin*TMath::Pi()/180);
double cosThetaMax=TMath::Cos(thetaMax*TMath::Pi()/180);
thetaCut<< cosThetaMax<<" < cosTheta && cosTheta < "<<cosThetaMin;
std::cout << thetaCut.str().c_str() << std::endl;

// cftDataTree->Draw("trkL:trkADC>>cftLengthvsADC",thetaCut.str().c_str());
// noMaskDataTree->Draw("trkL:trkADC>>noMaskLengthvsADC",thetaCut.str().c_str());
cftDataTree->Draw("trkL:trkADC>>cftLengthvsADC");
noMaskDataTree->Draw("trkL:trkADC>>noMaskLengthvsADC");
...
cftDataTree->Draw("trkL:trkADC>>cftLvsEn");
noMaskDataTree->Draw("trkL:trkADC>>noMaskLvsEn");
...
// char projname[123];
// for (int lbinStep = 75; lbinStep <= 425; lbinStep+=10){
// 	noMaskLvsEn->GetXaxis()->SetRange(0,1500);
// 	noMaskLvsEn->GetYaxis()->SetRange(0,500);
// 	noMaskLvsEn->Draw("colz");
// 	TH1D *px = noMaskLvsEn->ProjectionX("px", lbinStep, lbinStep);
// 	int xBuffer = 100;
// 	int xCutBin = px->FindLastBinAbove(10)+xBuffer;
// 	double xCut = px->GetBinCenter(xCutBin);
// 	double yCut = lbinStep/(double)100;
...
// 	TLine *linePlusBuffer = new TLine(xCut,0,xCut,px->GetBinContent(px->FindLastBinAbove(10)));
// 	linePlusBuffer->SetLineColor(kRed);
// 	linePlusBuffer->SetLineWidth(3);
// 	// px->GetXaxis()->SetRange(xCutBin-200,xCutBin+150);
// 	sprintf(projname,"projx.LvEn.yi%d.yf%d.zoom.png",lbinStep,lbinStep);
...
c1->SetLogz(1);
c1->SetLogx(1);
cftLvsEn->GetXaxis()->SetRange(100,30000);
cftLvsEn->Draw("Colz");
c1->SaveAs("LvEn.full.cft.log.png","recreate");
c1->SetLogx(0);
...
cftLvsEn->GetYaxis()->SetRange(0,600);
c1->Update();
c1->SaveAs("LvEn.lowEn.cft.plusCut.zoom.png","recreate");

c1->SetLogx(1);
noMaskLvsEn->GetXaxis()->SetRange(100,30000);
noMaskLvsEn->Draw("Colz");
c1->SaveAs("LvEn.full.noMask.log.png","recreate");
c1->SetLogx(0);
...
noMaskLvsEn->GetYaxis()->SetRange(0,600);
c1->Update();
...
cftFragsRebin->GetYaxis()->SetRange(1,noMaskFragsRebin->GetMaximum()*1.1);
