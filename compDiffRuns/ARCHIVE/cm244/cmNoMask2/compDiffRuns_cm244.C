...
else if ( !(strcmp(runType,"cmCFT2")))
{
	...
	runNum.push_back(2090);
	runNum.push_back(2091);
...
for (std::vector<int>::iterator runNumIter = runNum.begin(); runNumIter != runNum.end(); ++runNumIter)
{
	...
	c1->SaveAs(plotName);
	sprintf(histName,"lVsDigCharge{%s}",bestCut);
	lVsDigCharge->SetTitle(histName);
	tracks->Draw("length:digitCharge>>lVsDigCharge",bestCut,"colz");
	...
	c1->SaveAs(plotName);
	sprintf(histName,"digChargeVsAz{%s}",bestCut);
	digChargeVsAz->SetTitle(histName);
	tracks->Draw("digitCharge:direction.Phi()>>digChargeVsAz",bestCut,"colz");
	...
	c1->SaveAs(plotName);
	sprintf(histName,"digChargeVsPolar{%s}",bestCut);
	digChargeVsPolar->SetTitle(histName);
	tracks->Draw("digitCharge:direction.Theta()>>digChargeVsPolar",bestCut,"colz");
	...
	c1->SaveAs(plotName);
	sprintf(histName,"digChargeVsCosPolar{%s}",bestCut);
	digChargeVsCosPolar->SetTitle(histName);
	tracks->Draw("digitCharge:direction.CosTheta()>>digChargeVsCosPolar",bestCut,"colz");
