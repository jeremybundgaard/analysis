...
for (int treeEntry = 0; treeEntry < tracks->GetEntries(); treeEntry++){
  if(treeEntry%500000==0) printf("treeEntry:%d\n",treeEntry);
...
TTree* tracks_ghostTail_Flag = new TTree("tracks_ghostTail_Flag","tracks_ghostTail_Flag");
tracks_ghostTail_Flag->Branch("has_ghostTail",&has_ghostTail);
tracks_ghostTail_Flag->Branch("is_ghostTail",&is_ghostTail);
tracks_ghostTail_Flag->Branch("eventID",&eventID);
tracks_ghostTail_Flag->Branch("start",&start);
tracks_ghostTail_Flag->Branch("end",&end);
tracks_ghostTail_Flag->Branch("direction",&direction);
tracks_ghostTail_Flag->Branch("length",&length);
tracks_ghostTail_Flag->Branch("adc",&adc);
tracks_ghostTail_Flag->Branch("fitType",&fitType);
tracks_ghostTail_Flag->Branch("trackNo",&trackNo);
tracks_ghostTail_Flag->Branch("trackCount",&trackCount);
tracks_ghostTail_Flag->Branch("nDigits",&nDigits);
TTree* tracks_corrected = new TTree("tracks_corrected","tracks_corrected");
tracks_ghostTail_Flag->Branch("has_ghostTail",&has_ghostTail);
...
printf("is_ghostTail_counter:%d\thas_ghostTail_counter:%d\n",is_ghostTail_counter,has_ghostTail_counter);
fragCut_adc->Write();printf("fragCut_adc->Write() ");
tracks_corrected->Write();printf("tracks_corrected->Write()");
tracks_ghostTail_Flag->Write();printf("tracks_ghostTail_Flag->Write()");
TH2D* LvsADC = new TH2D("LvsADC","LvsADC",3000,100,30000,1000,0,7);
...
ellipse_canvas->SetLogz(1);ellipse_canvas->SetLogx(1);
tracks_ghostTail_Flag->Draw("length:adc>>LvsADC","is_ghostTail","colz");
ellipse_canvas->SaveAs("tracks_ghostTail_Flag_LvsADC.xghostTail.png");tracks_ghostTail_Flag->Draw("length:adc>>LvsADC","!is_ghostTail&&!(-1<direction.Phi()&&direction.Phi()<0)","colz");
ellipse_canvas->SaveAs("tracks_ghostTail_Flag_LvsADC.ghostTail_not.png");tracks_ghostTail_Flag->Draw("length:adc>>LvsADC","is_ghostTail||has_ghostTail","colz");
ellipse_canvas->SaveAs("tracks_ghostTail_Flag_LvsADC.is_ghostTail.has_ghostTail.png");tracks_ghostTail_Flag->Draw("length:adc>>LvsADC","!(-1<direction.Phi()&&direction.Phi()<0)","colz");
ellipse_canvas->SaveAs("tracks_ghostTail_Flag_LvsADC.png");tracks_corrected->Draw("length:adc>>LvsADC","!(-1<direction.Phi()&&direction.Phi()<0)","colz");
ellipse_canvas->SaveAs("tracks_corrected_LvsADC.png");tracks_corrected->Draw("length:adc>>h(3000,1000,30000,100,0,4)","!(-1<direction.Phi()&&direction.Phi()<0)","colz");
ellipse_canvas->SaveAs("tracks_corrected_LvsADC_logx.png");tracks_ghostTail_Flag->Draw("length:adc>>h(3000,1000,30000,100,0,4)","!(-1<direction.Phi()&&direction.Phi()<0)","colz");
ellipse_canvas->SaveAs("tracks_ghostTail_Flag_LvsADC_logx.png");ellipse_canvas->SetLogx(0);ellipse_canvas->SetLogz(0);
}
