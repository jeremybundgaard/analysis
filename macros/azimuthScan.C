...
void azimuthScan(const int preamp, const char* recoType, const int phiLoopMin, const int phiLoopMax, const int phiLoopStep) {
...
if (preamp==1)
{
	...
	if (!(strcmp(recoType,"hk"))){
		...
		sprintf(noMaskFileName,"$trees/hk.cfNoMask1.trees.root");
		sprintf(cftFileName,"$trees/hk.cfCFT1.trees.root");
	...
	if ( !(strcmp(recoType,"fp"))){
		...
		sprintf(noMaskFileName,"$trees/fp.cfNoMask1.trees.root");
		sprintf(cftFileName,"$trees/fp.cfCFT1.trees.root");
...
else if (preamp==2)
{
	...
	if (!(strcmp(recoType,"hk"))){
		sprintf(fitCut,"fitType==5");
		sprintf(noMaskFileName,"$trees/hk.cfNoMask2.trees.root");
		sprintf(cftFileName,"$trees/hk.cfCFT2.trees.root");
	...
	if ( !(strcmp(recoType,"fp"))){
		...
		sprintf(fitCut,"bestFit");
		sprintf(noMaskFileName,"$trees/fp.cfNoMask2.trees.root");
		sprintf(cftFileName,"$trees/fp.cfCFT2.trees.root");
...
for (int phi = phiLoopMin; phi < phiLoopMax; phi+=phiLoopStep){
    ...
    phiCut<< radPhiMin-TMath::Pi() <<" < direction.Phi() && direction.Phi() < "<< radPhiMax-TMath::Pi();
