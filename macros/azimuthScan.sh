...
if [[ $# != 2 ]]; then
	echo "USAGE: $macros/azimuthScan.sh <1|2> <hk|fp>"
	exit
fi
...
recoType=$2
phiStepSize=5
plotdir=`pwd`/azimuthScan.${preamp}preamp.${recoType}.reco.`date +%d%b%y_%H%M.%S`
mkdir $plotdir
cd $plotdir
...
for (( i = 0; i < 360; i+= 60 )); do
	...
	echo root -l -q -b azimuthScan.C\+\($preamp,\"$recoType\",$phiMin,$phiMax,$phiStepSize\)
	time root -l -q -b azimuthScan.C\+\($preamp,\"$recoType\",$phiMin,$phiMax,$phiStepSize\) >> $plotdir/log.output.phiMin${phiMin} &
	cd -
done
