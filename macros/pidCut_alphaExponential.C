...
void pidCut_alphaExponential(int preamp,const char* treeFile,const char* cuts="&&1") {
	...
	alphaCut_zCut_energy->SetPoint(pointNum,(px->FindFirstBinAbove(0)-1)*MeV_per_ADC_bin,yProj->FindFirstBinAbove(0)*0.01);
	alphaCut_zCut_energy->Write();
	alphaCut_zCut_adc->SetPoint(pointNum,(px->FindFirstBinAbove(0)-1)*MeV_per_ADC_bin*ADC_per_MeV,yProj->FindFirstBinAbove(0)*0.01);pointNum++;
	...
	// // 	energyCutPoint_frags_plusStdDev[arrayPos] = energyBin*MeV_per_ADC_bin;
	...
	// // 		double energy_plus=energyBin*MeV_per_ADC_bin;
	// // 		TLine *Line_plusStdDev = new TLine(
	// // 			energy_plus,length-0.02,
	// // 			energy_plus,length+0.02);
	// // 		Line_plusStdDev->SetLineColor(i+1);
	...
	for (int i = minStdDev; i < nStdDev; ++i){
		...
		for (int energyBin = 0; energyBin < 4000; ++energyBin){
			for (int lengthBin = 0; lengthBin < 1000; ++lengthBin){
				double energy=energyBin*MeV_per_ADC_bin;
				double length=lengthBin/(double)100;
	...
	for (int i = minStdDev; i < nStdDev; ++i)
	{
		...
		TH2D* noMaskLvEn_alphas_meanShifted_tmp = new TH2D(charBuff,charBuff,700,0,7,200,-100*MeV_per_ADC_bin,100*MeV_per_ADC_bin);
