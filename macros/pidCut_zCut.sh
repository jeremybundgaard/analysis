...
pidCut_zCut_plotdir=$cft/pid/zCut/pidCut_zCut_preamp${preamp}plots.`date +%d%b%y_%H%M.%S`
mkdir $pidCut_zCut_plotdir
cp $macros/pidCut_zCut.C $pidCut_zCut_plotdir
cd $pidCut_zCut_plotdir
echo root -l -q -b pidCut_zCut.C\+\($preamp,\"$noMaskTree\"\,\"$cuts\"\)>>log.stdout.txt
time root -l -q -b pidCut_zCut.C\+\($preamp,\"$noMaskTree\"\,\"$cuts\"\)>>log.stdout.txt
wait
