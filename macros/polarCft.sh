...
plotdir=`pwd`/polarCft.${actinide}.${preamp}preamp.pidType$pidType.pidCut$pidCut.`date +%d%b%y_%H%M.%S`
mkdir $plotdir
cd $plotdir
cp $macros/polarCft.{sh,C} $plotdir

echo root -l -q -b polarCft.C\+\($preamp,\"$actinide\"\,\"$noMaskTree\"\,\"$cftTree\"\,\"$cuts\"\,\"$pidType\"\)
time root -l -q -b polarCft.C\+\($preamp,\"$actinide\"\,\"$noMaskTree\"\,\"$cftTree\"\,\"$cuts\"\,\"$pidType\"\)
wait
