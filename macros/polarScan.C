...
void polarScan(const int preamp, const char* recoType, const int thetaLoopMin, const int thetaLoopMax, const int thetaLoopStep) {
...
char cftThetaCut[234];
char noMaskThetaCut[234];
...
for (int theta = thetaLoopMin; theta < thetaLoopMax; theta+=thetaLoopStep){
	...
	if (degThetaMax > 180) continue;

	thetaCut.str("");

    thetaCut<< radThetaMin <<" < direction.Theta() && direction.Theta() < "<< radThetaMax;

	sprintf(noMaskThetaCut,"%s && %s",thetaCut.str().c_str(),noMaskCut);
	sprintf(cftThetaCut,"%s && %s",thetaCut.str().c_str(),cftCut);

	std::cout << thetaCut.str().c_str() << std::endl;

	sprintf(charContainer,"noMaskLvEnPhiMin%03i",degThetaMin);
	TH2D* noMaskLvEn = new TH2D("noMaskLvEn",charContainer,40000,0,168.6,1000,0,10);

	sprintf(charContainer,"cftLvEnPhiMin%03i",degThetaMin);
	...
	cftLvEn->SetTitle(cftThetaCut);
	cftTracks->Draw("length:adc*168.6/40000>>cftLvEn",cftThetaCut);
	...
	noMaskLvEn->SetTitle(noMaskThetaCut);
	noMaskTracks->Draw("length:adc*168.6/40000>>noMaskLvEn",noMaskThetaCut);
