...
if [ $# -lt 1 ];then
echo "to run all events: ./doPadToPadCalibration.sh <infile> "
echo "to run N events: ./doPadToPadCalibration.sh <infile> <N>"
fi
...
# echo "niffte -c /g/g21/jerbundg/niffte/trunk/build/TPCBuildAverageBraggCurve.xml -i $infile -n $nEvents" >> $logfile

# echo "start building average bragg curves:   `date +%H%M.%S_%d%b%y` "
# niffte -c /g/g21/jerbundg/niffte/trunk/build/TPCBuildAverageBraggCurve.xml -i $infile -n $nEvents >> $logfile
...
# echo "niffte -c /g/g21/jerbundg/niffte/trunk/build/TPCPadToPadCalibration.xml -i $infile -n $nEvents" >> $logfile

# echo "start computing scale factors:   `date +%H%M.%S_%d%b%y` "
# niffte -c /g/g21/jerbundg/niffte/trunk/build/TPCPadToPadCalibration.xml -i $infile -n $nEvents >> $logfile
